...
// --- Gate: :usage depends on nothing but the Kotlin stdlib (ARCHITECTURE.md §2) ---
...
val checkUsagePurity by tasks.registering {
    val artifacts =
    ...
    doLast {
        ...
        require(extra.isEmpty() && android.isEmpty()) {
            buildString {
                append("usage must depend on nothing but the Kotlin standard library")
...
// --- Gate: :usage is a leaf; its test classpath sees no other project (ARCHITECTURE.md §2) ---
val checkUsageIsLeaf by tasks.registering {
    ...
    doLast {
        require(projects.get() == setOf(":usage")) {
            "usage must not depend on another project, found: ${projects.get() - ":usage"}"
        }
    }
}
...
val checkUsageApiTypes by tasks.registering {
    val apiFile = layout.projectDirectory.file("api/usage.api").asFile
    inputs.file(apiFile)
    doLast {
        ...
        require(offenders.isEmpty()) {
            "public API of :usage names types outside kotlin.* and the project package: $offenders"
        }
    }
}

tasks.named("check") {
    dependsOn(checkUsagePurity, checkUsageIsLeaf, checkUsageApiTypes)
}
